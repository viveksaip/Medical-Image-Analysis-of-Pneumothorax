...
Original file is located at
    https://colab.research.google.com/drive/1N46oW07q5azrgegG9pmvhESlAcsuFrW0
"""
...
import os
os.chdir('/content/gdrive/My Drive/DDP')
...
for full_fn in masks_train_fn:
    fn = full_fn.split('/')[-1]
    if fn != '1.2.276.0.7230010.3.1.4.8323329.2633.1517875173.805125.png':
      shutil.move(full_fn,os.path.join(train_dir,fn))
...
batch_size = 16
swa = SWA('./keras_swa.model',2) #Vchange #67
...
grid_width = 1 #V
grid_height = int(max_images / grid_width)
fig, axs = plt.subplots(grid_height, grid_width, figsize=(4, 4)) #V
...
for i,(im, mask) in enumerate(zip(images,masks)):
    if i==19:
        ...
        plt.imshow(np.array(np.round(pred > threshold_best), dtype=np.float32), alpha=0.5, cmap="Greens")
        break
plt.suptitle("Green:Prediction , Red: Pneumothorax.")
